...
class Component(ComponentBase):
    ...
    def run(self):
        ...
        start_date = datetime.datetime.combine(datetime.datetime.utcnow(), datetime.time(00, 00, 00)) - datetime.timedelta(days = DAYS_COUNT)
        ...
        ## obtain data
        api_client = pc2.api_client.ApiClient(host=params.get(KEY_CLOUD_URL)).get_client_credentials_token(params.get(KEY_CLIENT_ID), params.get(KEY_PASSWORD))
        ...
        if responses.conversations is None:
            # @todo: prepare empty tables
            sys.exit(0)
        ...
        conversation_table = self.create_out_table_definition('conversations.csv', incremental=True, primary_key=['conversation_id'])
        with open(conversation_table.full_path, mode='wt', encoding='utf-8', newline='') as out_file:
            writer = csv.DictWriter(out_file, fieldnames=['conversation_id', 'conversation_start', 'conversation_end'])
            writer.writeheader()
            for line in output['conversations']:
                writer.writerow({key: value for key, value in line.items() if key not in ['agents', 'wrap_up_code'] })
        self.write_manifest(conversation_table)

        # Create output table - agents
        agents_table = self.create_out_table_definition('agents.csv', incremental=True, primary_key=['conversation_id', 'agent_email'])
        with open(agents_table.full_path, mode='wt', encoding='utf-8', newline='') as out_file:
            writer = csv.DictWriter(out_file, fieldnames=['conversation_id', 'agent_email'])
            writer.writeheader()
            for line in output['conversations']:
        ...
        wrap_table = self.create_out_table_definition('wrap_up_code.csv', incremental=True, primary_key=['conversation_id', 'wrap_up_code'])
